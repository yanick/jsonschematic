...
<Story name="array">
    {#each examples.filter(({ type }) => type === 'array') as definition (definition.title)}
        <Schema {definition} />
    {/each}
</Story>

<Story name="the rest">
    {#each examples.filter(({ type }) => !['array', 'string', 'boolean', 'number'].includes(type)) as definition (definition.title)}
        <Schema {definition} />
    {/each}
</Story>
