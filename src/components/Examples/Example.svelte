...
<div>
    {#if edit_mode}
        <textarea class:error={error_edit} cols="60" rows="10"
        bind:value={text_def} />
        {#if error_edit }
        <div class="error">Invalid JSON</div>
        {/if}
    {:else}
    <pre><code>{ text_def }</code></pre>
    <div class="edit_me">
    <input type="button" value="edit me" on:click={() => edit_mode =!edit_mode
    }/>
    </div>
  {/if}
</div>
  ...
  <div class="instance"> <Instance top_level definition={edited_def} /> </div>
</div>
